...
import com.example.workcalendar.DataModel.DAO.WorkDayDAO;
...
public class SQLLite_WorkDays implements WorkDayDAO {
    ...
    public void insertWorkDay(WorkDay workDay) {
        ...
        values.put("DAY",workDay.getWorkdate());
    ...
    public WorkDay getWorkDay(int id) {
        ...
        Cursor cursor = db.query(DataBase.WorkTable, new String[]{"ID", "DAY", "SATURDAY", "USERID", "SERVERID"}, "ID=?", new String[]{String.valueOf(id)}, null, null, null);
        WorkDay day=new WorkDay();
        if(cursor.moveToFirst())
        {
            day.setId(cursor.getInt(cursor.getColumnIndex("ID")));
            long _date = cursor.getLong(cursor.getColumnIndex("DAY"));
            day.setWorkdate(_date);
            ...
            day.setServerId(cursor.getString(cursor.getColumnIndex("SEVERID")));
    ...
    private ArrayList<WorkDay> _getWorkDays(int user_id, boolean byUser, Boolean sortedCurrentDate) {
        ...
        if(sortedCurrentDate)
        {
         ...
         cursor = db.query(DataBase.WorkTable, new String[]{"ID", "DAY", "SATURDAY", "USERID","SERVERID"}, selections, args, null, null, "DAY");
        ...
        while (cursor.moveToNext()) {
            ...
            long date = cursor.getLong(cursor.getColumnIndex("DAY"));
            _day.setWorkdate(date);
            ...
            _day.setServerId(cursor.getString(cursor.getColumnIndex("SEVERID")));
            workDays.add(_day);
        }
        dataBase.close();
        return  workDays;
  ...
  {
      Cursor cursor=db.query(DataBase.UsersTable,new String[]{"USERID","NAME","PHONE","BIRTHDAY","SERVERID"},"USERID=?",new String[]{String.valueOf(user_id)},null,null,null);
    ...
    public ArrayList<WorkDay> getUserNextWorkDay(int user_id, Date afterDate) {
        ...
        String orderBy="DAY";
        ArrayList<WorkDay> workDays=new ArrayList<WorkDay>();
        Cursor cursor = db.query(DataBase.WorkTable, new String[]{"ID", "DAY", "SATURDAY", "USERID","SERVERID"}, "USERID=? and DAY>?", selecArgs, null, null,orderBy);
        if (cursor.moveToNext())
        {
            nextWorkDay= new WorkDay();
            nextWorkDay.setId(cursor.getInt(cursor.getColumnIndex("ID")));
            long _date = cursor.getLong(cursor.getColumnIndex("DAY"));
            nextWorkDay.setWorkdate(_date);
            nextWorkDay.setSaturday(Boolean.valueOf(cursor.getString(cursor.getColumnIndex("SATURDAY"))));
            nextWorkDay.setServerId(cursor.getString(cursor.getColumnIndex("SERVERID")));
    ...
    public boolean checkExists(Date date) {
        ...
        DataBase dataBase = new DataBase(context);
        SQLiteDatabase db = dataBase.getReadableDatabase();
        Cursor cursor = db.query(DataBase.WorkTable, new String[]{"ID", "DAY", "SATURDAY", "USERID","SERVERID"}, "DAY=?", new String[]{String.valueOf(date.getTime())}, null, null,null);
        if(cursor.moveToFirst())
        {
            return true;
        }else return false;
